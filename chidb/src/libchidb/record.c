...
int chidb_DBRecord_appendInt8(DBRecordBuffer *dbrb, int8_t v)
{
	...
	if (dbrb->offset + 1 > dbrb->buf_size) { dbrb->buf_size += 1024; dbrb->dbr->data = realloc(dbrb->dbr->data, dbrb->buf_size); }
...
int chidb_DBRecord_appendInt16(DBRecordBuffer *dbrb, int16_t v)
{
	...
	if (dbrb->offset + 2 > dbrb->buf_size) { dbrb->buf_size += 1024; dbrb->dbr->data = realloc(dbrb->dbr->data, dbrb->buf_size); }
...
int chidb_DBRecord_appendInt32(DBRecordBuffer *dbrb, int32_t v)
{
	...
	if (dbrb->offset + 4 > dbrb->buf_size) { dbrb->buf_size += 1024; dbrb->dbr->data = realloc(dbrb->dbr->data, dbrb->buf_size); }
...
int chidb_DBRecord_appendString(DBRecordBuffer *dbrb,  char *v)
{
	...
	len = strlen(v);
	if (dbrb->offset + len > dbrb->buf_size) { dbrb->buf_size += 1024; dbrb->dbr->data = realloc(dbrb->dbr->data, dbrb->buf_size); }
...
int chidb_DBRecord_create(DBRecord **dbr, const char *spec, ...)
{
	...
	*dbr = malloc(sizeof(DBRecord));
    ...
    {
    	char intsize;
		char *s; uint8_t i8; uint16_t i16; uint32_t i32;
